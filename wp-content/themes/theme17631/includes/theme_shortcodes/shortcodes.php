...
function shortcode_recent_posts($atts, $content = null) {
		...
		foreach($latest as $post) {
				...
				$teamjob = $custom["team-job"][0];

				if ($teamjob !='') {
					$output .= '<span class="job">';
					$output .= $teamjob;
					$output .= '</span>';
				}
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme1763");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme1763");
